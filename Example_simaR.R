...
New_data<-simData(Means,20,50) #Simulating 50 curves with 20 samples per initial prey density#
...
plot(Means, pch=16)
Means[,2]<-Means[,2]/Means[,1] #Calculating the average proportion of prey killed at each initial prey density#
New_Curves_real77r<-simData(Means,20,100) #Simulating 100 curves with 20 samples per initial prey density#
...
#Depending on the version, "real77" and "real77r" will not work, if so, try "flexp" and "flepxnr" that are exactly the same#
...
#match(boxplot.stats(rates_real77r)$out,rates_real77r) #trick to check for outliers
plot(rates_real77r)
...
par(family="serif",font=1,cex=1.1,mar=c(3,5,1,4)) #Plotting the simulated-MAR and its confidence intervals#
